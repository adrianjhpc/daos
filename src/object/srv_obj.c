...
static int
obj_set_reply_sizes(crt_rpc_t *rpc, daos_iod_t *iods, uint32_t iod_nr)
{
	struct obj_rw_in	*orw = crt_req_get(rpc);
	struct obj_rw_out	*orwo = crt_reply_get(rpc);
	uint64_t		*sizes = NULL;
	int			size_count = iod_nr;
	int			i;
	...
	if (orwo->orw_iod_sizes.ca_count != 0) {
		D_ASSERT(orwo->orw_iod_sizes.ca_count == size_count);
		return 0;
	}
		      ...
		      uint64_t *split_offs, struct dtx_handle *dth)
{
	...
	if (obj_rpc_is_update(rpc)) {
		...
		if (ec_deg_fetch && !spec_fetch) {
			...
			/* Dup the iods to make it reentrant, as the
