	...
	if (ENCODING(proc_op)) {
		...
		if (off_nr != 0) {
			...
			rc = crt_proc_memcpy(proc, iod_array->oia_offs,
					     off_nr * sizeof(iod_array->oia_offs[0]));
			if (unlikely(rc)) {
				       ...
				       struct dtx_redundancy_group *p)
{
	...
	rc = crt_proc_uint16_t(proc, &p->drg_redundancy);
	if (unlikely(rc))
		return rc;

	rc = crt_proc_uint16_t(proc, &p->drg_flags);
	if (unlikely(rc))
		return rc;
...
crt_proc_daos_iod_t(crt_proc_t proc, daos_iod_t *iod)
{
	...
	case CRT_PROC_ENCODE:
		rc = crt_proc_memcpy(proc, iod->iod_recxs,
				     iod->iod_nr * sizeof(iod->iod_recxs[0]));
		if (unlikely(rc)) {
			if (DECODING(proc_op))
				D_FREE(iod->iod_recxs);
			return rc;
				 ...
				 struct daos_cpd_sub_req *dcsr, bool with_oid)
{
	...
	case DCSO_UPDATE: {
		...
		rc = crt_proc_memcpy(proc, dcu->dcu_ec_tgts,
				     dcsr->dcsr_ec_tgt_nr * sizeof(dcu->dcu_ec_tgts[0]));
		if (unlikely(rc))
			D_GOTO(out, rc);
