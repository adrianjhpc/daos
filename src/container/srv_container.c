 ...
 */
int
ds_cont_update_agg_eph(uuid_t pool_uuid, uuid_t cont_uuid,
		       d_rank_t rank, daos_epoch_t eph)
{
	...
	D_DEBUG(DB_MD, DF_CONT" update agg eph rank %u eph "DF_U64"\n",
		DP_CONT(pool_uuid, cont_uuid), rank, eph);
	if (i == ec_agg->ea_servers_num)
		D_WARN("rank %u eph "DF_U64" does not exist for "DF_CONT"\n",
			rank, eph, DP_CONT(pool_uuid, cont_uuid));
out_put:
...
cont_refresh_vos_agg_eph_one(void *data)
{
	...
	cont_child->sc_aggregation_max = arg->min_eph;
	ds_cont_child_put(cont_child);
	return rc;
}

int
ds_cont_refresh_agg_eph(uuid_t pool_uuid, uuid_t cont_uuid, daos_epoch_t eph)
{
